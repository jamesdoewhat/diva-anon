...
# Common args for alchemy env

def get_common_arg_parser():
    parser = argparse.ArgumentParser(conflict_handler='resolve')

    # --- GENERAL ---
    parser.add_argument('--num-frames', type=int_arg, default=8e6, help='number of frames to train')
    parser.add_argument('--trials-per-episode', type=int, default=10, help='number of MDP trials for adaptation')
    parser.add_argument('--max-steps', type=int, default=20)
    parser.add_argument('--test-env-name', default='AlchemyRandom-v0', help='environment to test on')

    # --- ALCHEMY ---
    parser.add_argument('--gt-type', default='s8-d-d-d-c6', help='Genotype type')

    # --- PLR --- (from DCD settings for car racing--haven't modified sinse)
    ...
    parser.add_argument("--plr-seed-buffer-size",type=int,default=112_500, help="Number of levels to store in our buffer, after which we replace")
    ...
    parser.add_argument('--policy-num-steps', type=int, default=40, help='number of env steps to do (per process) before updating')
    ...
    parser.add_argument('--size-vae-buffer', type=int, default=100000, help='how many trajectories (!) to keep in VAE buffer')
    ...
    parser.add_argument('--latent-dim', type=int, default=10, help='dimensionality of latent space')
    ...
    parser.add_argument('--eval-save-video', type=bool_arg, default=False, help='save video of rollouts during each evaluation')
    parser.add_argument('--video-fps', type=int, default=3, help='fps for video saving')
