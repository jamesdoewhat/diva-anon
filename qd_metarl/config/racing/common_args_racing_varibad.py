...
def get_common_arg_parser():
    ...
    parser.add_argument('--test-env-name', default='CarRacing-Bezier-v0', help='environment to train on')
    ...
    parser.add_argument('--policy-activation-function', type=str, default='relu', help='tanh/relu/leaky-relu')
    parser.add_argument('--policy-initialisation', type=str, default='orthogonal', help='normc/orthogonal')
    parser.add_argument('--vae-subsample-elbos', type=int, default=100, help='for how many timesteps to compute the ELBO; None uses all')
    parser.add_argument('--vae-subsample-decodes', type=int, default=100, help='number of reconstruction terms to subsample; None uses all')
    parser.add_argument('--video-fps', type=int, default=15, help='fps for video saving')
    parser.add_argument('--racing-fps', type=int, default=15, help='fps for racing env')
    parser.add_argument('--gt-type', default='CP-1', help='Genotype type')

    # --- PLR --- (from DCD settings for car racing)
    parser.add_argument("--plr-level-replay-score-transform", type=str, default='power', choices=['constant', 'max', 'eps_greedy', 'rank', 'power', 'softmax'], help="Level replay scoring strategy")
    ...
    parser.add_argument("--plr-level-replay-strategy", type=str,default='positive_value_loss',
    ...
    parser.add_argument("--plr-level-replay-schedule",type=str,default='fixed', help="Level replay schedule for sampling seen levels")
    parser.add_argument("--plr-level-replay-rho",type=float, default=0.5, help="Minimum size of replay set relative to total number of levels before sampling replays.")
    parser.add_argument("--plr-replay-prob", type=float,default=0.5, help="Probability of replaying a level instead of a new level.")
    parser.add_argument("--plr-level-replay-alpha",type=float, default=0.0, help="Level score EWA smoothing factor")
    parser.add_argument("--plr-staleness-coef",type=float, default=0.7, help="Staleness weighing")
    ...
    parser.add_argument("--plr-seed-buffer-size",type=int,default=8000, help="Number of levels to store in our buffer, after which we replace")
    parser.add_argument("--plr-weight-log-interval", type=int, default=10, help="Save level weights every this many updates")
    ...
    # PPO specific
    parser.add_argument('--ppo-num-epochs', type=int, default=8, help='number of epochs per PPO update')
    ...
    parser.add_argument('--ppo-clip-param', type=float, default=0.01, help='clamp param')

    # other hyperparameters
    parser.add_argument('--lr-policy', type=float, default=0.0007, help='learning rate (default: 7e-4)')
    parser.add_argument('--num-processes', type=int, default=4, help='how many training CPU processes / parallel environments to use (default: 16)')
    parser.add_argument('--policy-num-steps', type=int, default=500, help='number of env steps to do (per process) before updating')
    ...
    parser.add_argument('--disable-kl-term', type=bool_arg, default=False, help='dont use the KL regularising loss term')
    ...
    parser.add_argument('--rlloss-through-encoder', type=bool_arg, default=False, help='backprop rl loss through encoder')
    parser.add_argument('--add-nonlinearity-to-latent', type=bool_arg, default=False, help='Use relu before feeding latent to policy')
    ...
    parser.add_argument('--log-interval', type=int, default=10, help='log interval, one log per n updates')
    ...
    # parser.add_argument('--video-fps', type=int, default=15, help='fps for video saving')
    parser.add_argument('--vis-interval', type=int, default=500, help='visualisation interval, one viz per n updates')
