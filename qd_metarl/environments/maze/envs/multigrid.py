...
import qd_metarl.environments.maze.window as window
import numpy as np
...
class Door(minigrid.Door):
    ...
    def toggle(self, env, pos, carrying):
        ...
        if self.is_locked:
            if isinstance(carrying,
                          minigrid.Key) and carrying.color == self.color:
...
class Grid(minigrid.Grid):
    """Extends Grid class, overrides some functions to cope with multi-agent
    case."""
    ...
    def render_tile(
        ...
        # Render agents after highlight to avoid highlighting agent triangle
        # (the combination of colours makes it difficult to ID agent)
        if obj is not None and obj.type == "agent":
            obj.render(img)
    ...
    def render(self, tile_size, highlight_mask=None):
        ...
        Args:
            ...
            highlight_mask: An array of binary masks, showing which part of the
                grid should be highlighted for each agent. Can also be used in
                partial observation for single agent, which must be handled
                differently.
        ...
        for y in range(0, self.height):
            for x in range(0, self.width):
                ...
                if isinstance(highlight_mask, list):
                    ...
                    highlights = [
                        highlight_mask[a][x, y] for a in range(n_agents)
                    ]
                else:
...
class MultiGridEnv(minigrid.MiniGridEnv):
    ...
    def __init__(
        ...
        Args:
            grid_size: Number of tiles for the width and height of the square
                grid.
            ...
            competitive: If True, as soon as one agent locates the goal, the
                episode ends for all agents. If False, if one agent locates the
                goal it is respawned somewhere else in the grid, and the episode
                continues until max_steps is reached.
            fixed_environment: If True, will use the same random seed each time
                the environment is generated, so it will remain constant / be
        ...
        self.direction_obs_space = gym.spaces.Box(low=0,
                                                  high=3,
                                                  shape=(self.n_agents,),
                                                  dtype="uint8")

        self.pos_obs_space = gym.spaces.Box(low=0,
                                            high=grid_size,
                                            shape=(self.n_agents,),
                                            dtype="uint8")
        ...
        if self.minigrid_mode:
            ...
            self.image_obs_space = gym.spaces.Box(
                low=0,
                high=255,
                shape=(self.n_agents, self.agent_view_size,
                       self.agent_view_size, 3),
                dtype="uint8",
            )

        # Observations are dictionaries containing an encoding of the grid and
        # the agent's direction. x, y are agents' position that are only used by
        # surrogate model.
    ...
    def place_obj(self,
        ...
        Args:
            ...
            top: (x,y) position of the top-left corner of rectangle where to
                place.
            size: Size of the rectangle where to place.
            reject_fn: Function to filter out potential positions.
            max_tries: Throw an error if a position can't be found after this
                many tries.
    ...
    def place_agent(self,
        ...
        Args:
            top: (x,y) position of the top-left corner of rectangle where agents
                can be placed.
            size: Size of the rectangle where to place.
    ...
    def place_one_agent(
        ...
        self.place_agent_at_pos(agent_id,
                                pos,
                                agent_obj=agent_obj,
                                rand_dir=rand_dir)
    ...
    @property
    def dir_vec(self):
        """Get the direction vector for the agent (points toward forward
        movement).
    ...
    def agent_is_done(self, agent_id):
        ...
        # If an agent finishes the level while carrying an object, it is
        # randomly respawned in a new position. Warning: this may break
        # dependencies for the level (e.g. if a key is spawned on the wrong side
        # of a door).
    ...
    def move_agent(self, agent_id, new_pos):
        ...
        assert self.grid.get(new_pos[0],
                             new_pos[1]).cur_pos == self.agent_pos[agent_id]
    ...
    def gen_obs(self):
        ...
        for a in range(self.n_agents):
            ...
            xs.append(self.agent_pos[a][0])
            ys.append(self.agent_pos[a][1])

        # Backwards compatibility: if there is a single agent do not return an
        # array
        if self.minigrid_mode:
            images = images[0]
        ...
        # - x, y positions of agents (only for the surrogate model)
        # TODO(costales): Why only xs and ys used for surrogate model?
        obs = {"image": images,      # Partially-observable view of env
               "direction": dirs,    # Direction gent is facing
               "x": xs,              # X pos of agent
               "y": ys}              # Y pos of agent
        # obs = images.flatten()  # OLD: for compatibility with OG VariBAD code

        return obs

    def gen_agent_obs(self, agent_id):
        """Generate the agent's view (partially observed, low-resolution
        encoding).
        ...
        Returns:
            3-dimensional partially observed agent-centric view, and int
            direction
        """
