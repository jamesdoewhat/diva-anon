...
from qd_metarl.environments.box2d.car_dynamics import Car
# from gym.envs.box2d.car_dynamics import Car
from gym.utils import seeding, EzPickle
from qd_metarl.environments.box2d import geo_complexity
from qd_metarl.qd.measures.car_racing_measures import CarRacingMeasures
...
class CarRacingBezier(gym.Env, EzPickle):
    ...
    # TODO: fix metadata (need to generate dynamically)
    ...
    def __init__(self,
        ...
        - variable_trial_lengths (bool): Whether to end the episode early.
        ...
        if self.use_f1_tracks:
            # global racetracks; 
            from . import racetracks
            from qd_metarl.environments.box2d.car_racing_f1 import RACETRACKS
    ...
    def set_genotype_info(self):
        ...
        # TODO(costales): see if this seems appropriate given the natural 
        # control points that are created with seeding. You might want to
        # change this to have it better reflect the natural distribution.
        self.genotype_lower_bounds = np.array([0.0] * self.genotype_size)
        self.genotype_upper_bounds = np.array([1.0] * self.genotype_size)
    ...
    def compute_measures_static(genotype=None, measures=None, gt_type=None, return_pg=False):
        ...
        size = len(genotype)  # TODO: seems unnecessary?
        pg = CarRacingBezier.process_genotype(genotype, size, gt_type)
    ...
    def compute_measures(self, genotype=None, measures=None, return_pg=False):
        ...
        if type(self.genotype) == str and self.genotype == 'f1':
            genotype = None
    ...
    def genotype_from_seed_static(
        ...
        while True:
            ...
            pg = CarRacingBezier.process_genotype(sol, genotype_size, gt_type=gt_type)
    ...
    def genotype_from_seed(self, seed, level_store=None):
        ...
        # TODO: Create function that can be called outside of this class and
        # MazeEnv, since the code is so similar---pass in sol generator, 
        # since this part is different (rng.uniform vs rng.integers)
    ...
    @staticmethod
    def process_genotype(genotype, size, gt_type='CP-1'):
        """ Extract information from genotype and genotype type. """
        gisn = genotype is None
    ...
    def generate_level_from_genotype(self, genotype, gt_type='CP-1'):
        ...
        pg = self.process_genotype(
            genotype, size=self.genotype_size, gt_type=gt_type)
    ...
    def set_genotype_from_current_task(self):
        ...
        if 'CP' in self.gt_type:
            ...
            raise ValueError(f'Unknown genotype type: {self.gt_type}')
        return self.process_genotype_new(self.genotype)
    ...
    def reset_task(self, task=None) -> None:
        ...
        self._create_track()  # TODO: make sure it uses the current seeds
    ...
    def step(self, action):
        ...
        # End episiode if beyond max_steps, regardless of not self.variable_trial_lengths
        # (i.e. we always return done if we're past max steps)
        done_to_return = done
        if self.max_steps is not None and self.steps > self.max_steps:
            done_to_return = True
        # Otherwise, if we've completed or crashed...
        elif done and self.variable_trial_lengths:
            done_to_return = True
        elif done and not self.variable_trial_lengths:
    ...
    def render(self, mode="human"):
        ...
        if self.viewer is None:
            from qd_metarl.environments.box2d import rendering
