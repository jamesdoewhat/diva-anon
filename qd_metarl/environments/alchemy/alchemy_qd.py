...
from qd_metarl.environments.alchemy.alchemy_gym import SymbolicAlchemyGymEnv
from qd_metarl.environments.alchemy.alchemy_gym import get_chemistry_from_seed
from qd_metarl.environments.alchemy.alchemy_gym import get_items_from_seed
from qd_metarl.environments.alchemy.alchemy_gym import AlchemyMods
from qd_metarl.environments.alchemy.alchemy_gym import get_fixed_chemistries
from qd_metarl.qd.measures.alchemy_measures import AlchemyMeasures
from qd_metarl.environments.alchemy.genotypes.helpers import GRAPH_LIST
from qd_metarl.environments.alchemy.genotypes.default import DefaultGenotype
import qd_metarl.environments.alchemy.genotypes.custom  # TODO: why?
from qd_metarl.environments.alchemy.genotypes.custom import CustomGenotype
...
class ExtendedSymbolicAlchemy(SymbolicAlchemyGymEnv):
    def __init__(self,
        ...
        self.fixed_environment = False  # TODO
    ...
    def set_genotype_info(self):
        ...
        self.measures = AlchemyMeasures.get_all_measures()  # TODO
    ...
    def genotype_from_seed(self, seed, level_store=None):
        ...
        while True:
            ...
            genotype = pg.genotype
            del reason  # TODO: if failed after 1000, print most common reason
            if valid:
                break
            if num_attempts == 100:
    ...
    def generate_level_from_genotype(self, genotype, only_return=False):
        ...
        processed_genotype = self.process_genotype_new(genotype)
    ...
    def reset(self, trial_num=0) -> np.ndarray:
    ...
    def generated_items_fn(self, trial_num=None, use_passed_trial_num=False):
        """ Return the generated items. """
        if use_passed_trial_num:
            trial_num = trial_num
        else:
            trial_num = self.current_trial_num
        
        # Called by SymbolicAlchemyEnv, which currently
        # doesn't have correct trial_numbers tracked; we track it here 
        # independently
        # TODO: Align trial numbers between this class and SymbolicAlchemy
        return self._generated_items[trial_num * int(self.use_dynamic_items)]
    ...
    def reset_task(self, task=None) -> None:
        ...
        self.env_items = [self.generated_items_fn(i, use_passed_trial_num=True) for i in range(self.gt.num_item_specs)]

        self.reset(trial_num=0)
