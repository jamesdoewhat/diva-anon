...
from dm_alchemy.types.stones_and_potions import StoneMap
from dm_alchemy.types.stones_and_potions import PotionMap
from dm_alchemy.types.stones_and_potions import LatentStone
from dm_alchemy.types.stones_and_potions import latent_potion_from_index
from dm_alchemy.types.stones_and_potions import possible_rotations
from dm_alchemy.types import utils as type_utils
from dm_alchemy.types.graphs import no_bottleneck_constraints
from dm_alchemy.types.graphs import create_graph_from_constraint
from dm_alchemy.types.graphs import constraint_from_graph
from qd_metarl.environments.alchemy.alchemy_gym import MAX_STONES, MAX_POTIONS
from qd_metarl.environments.alchemy.alchemy_gym import LATENT_POTION_LIST
from qd_metarl.environments.alchemy.alchemy_gym import potion_to_latent_potion
from qd_metarl.environments.alchemy.genotypes.helpers import IDX_TO_PERMUTATION
from qd_metarl.environments.alchemy.genotypes.helpers import PERMUTATION_TO_IDX
from qd_metarl.environments.alchemy.genotypes.helpers import GRAPH_LIST
        ...
        if gisn:
            ...
            if self.gt_scs[0] == 'd':
                ...
                for _ in range(self.num_item_specs):
                    num_stones.append(genotype[idx]); idx += 1
                stone_latent_states = genotype[idx : idx + MAX_STONES * 3 * self.num_item_specs]; idx += MAX_STONES * 3 * self.num_item_specs
            elif 's' in self.gt_scs[0]:
                n = int(self.gt_scs[0][1:])  # How many bits for each stone
                num_stones = []
                for _ in range(self.num_item_specs):
                    num_stones.append(genotype[idx]); idx += 1
                raw_stone_latent_states = genotype[idx: idx + n * 3 * MAX_STONES * self.num_item_specs]; idx += n * 3 * MAX_STONES * self.num_item_specs
            ...
            if self.gt_scs[1] == 'd':
                num_potions = []
                for _ in range(self.num_item_specs):
                    num_potions.append(genotype[idx]); idx += 1
                potion_effects = genotype[idx : idx + MAX_POTIONS * self.num_item_specs]; idx += MAX_POTIONS * self.num_item_specs
                potion_effects = np.reshape(potion_effects, (self.num_item_specs, MAX_POTIONS))
            
            # (c) Stone chemistry
            if self.gt_scs[2] == 'd':
                stone_rotation = genotype[idx : idx + 1]; idx += 1
                stone_reflection = genotype[idx : idx + 3]; idx += 3
            
            # (d) Potion chemistry
            if self.gt_scs[3] == 'd':
                potion_reflection = genotype[idx : idx + 3]; idx += 3
                potion_permutation = genotype[idx : idx + 1]; idx += 1
            
            # (e) Graph topology
            if self.gt_scs[4] == 'd':  
                graph_topology_idx = genotype[idx : idx + 1]; idx += 1
                ...
                for i in indices:
                    constraints[i] = CONSTRAINT_INT_TO_STR[genotype[idx]]; idx += 1
                ...
                graph = create_graph_from_constraint(constraints)  # TODO: Is it okay we're not passing in potions?
