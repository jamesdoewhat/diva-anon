# NOTE: this file modifies/extends dm_alchemy/symbolic_alchemy.py
import functools
import os
import collections
...
from dm_alchemy.types.stones_and_potions import latent_stone_from_index
from dm_alchemy.types.stones_and_potions import stone_map_from_index
from dm_alchemy.types.stones_and_potions import possible_rotations
from dm_alchemy.types.stones_and_potions import potion_map_from_index
...
from qd_metarl.environments.env_utils.gym_wrapper import GymFromDMEnv
from qd_metarl.environments.alchemy import symbolic_alchemy
...
class SymbolicAlchemyGymEnv(GymFromDMEnv):
    ...
    def __init__(self, 
        ...
        # TODO: Only visualize for certain settings of hyperparams
        
        if self._visualize:
            from qd_metarl.environments.alchemy.visualizer import AlchemyVisualizer
    ...
    def step(self, action):
        ...
        if self._current_episode_steps >= self._max_trial_steps:
            done = True
...
def get_chemistry_from_seed(seed: int, 
    ...
    if am is not None and am.fix_stone_map:
        stone_map_gen = lambda: stone_map_from_index(0)
    else:
    ...
    if am is not None and am.fix_potion_map:
        ...
        potion_map_gen = lambda: potion_map_from_index(0, index_to_perm_index)
    else:
