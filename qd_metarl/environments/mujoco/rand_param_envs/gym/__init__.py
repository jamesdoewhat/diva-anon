...
from qd_metarl.environments.mujoco.rand_param_envs.gym import error
from qd_metarl.environments.mujoco.rand_param_envs.gym.configuration import logger_setup, undo_logger_setup
from qd_metarl.environments.mujoco.rand_param_envs.gym.utils import reraise
from qd_metarl.environments.mujoco.rand_param_envs.gym.version import VERSION as __version__
...
def sanity_check_dependencies():
    ...
    #         numpy.__version__)
    if packaging.version.Version(numpy.__version__) < packaging.version.Version('1.10.4'):
        logger.warn(
            "You have 'numpy' version %s installed, but 'gym' requires at least 1.10.4. HINT: upgrade via 'pip install -U numpy'.",
            numpy.__version__)
    ...
    if packaging.version.Version(requests.__version__) < packaging.version.Version('2.0'):
        logger.warn(
            "You have 'requests' version %s installed, but 'gym' requires at least 2.0. HINT: upgrade via 'pip install -U requests'.",
            requests.__version__)
...
from qd_metarl.environments.mujoco.rand_param_envs.gym.core import Env, Space, Wrapper, \
    ObservationWrapper, ActionWrapper, RewardWrapper
from qd_metarl.environments.mujoco.rand_param_envs.gym.benchmarks import benchmark_spec
from qd_metarl.environments.mujoco.rand_param_envs.gym.envs import make, spec
from qd_metarl.environments.mujoco.rand_param_envs.gym import wrappers

# QD+MetaRL: My line (why wasn't this here?)
from qd_metarl.environments.mujoco.rand_param_envs.gym import spaces 
