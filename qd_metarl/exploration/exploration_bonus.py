# Modified from https://github.com/lmzintgraf/hyperx/tree/main/exploration
import numpy as np
import torch

from qd_metarl.exploration.rnd.rnd_bonus import RNDRewardBonus
from qd_metarl.exploration.rollout_storage import RolloutStorage
...
class ExplorationBonus(object):
    def __init__(self,
        ...
        if self.args.state_expl_idx is None:
            self.state_expl_idx = torch.tensor([i for i in range(self.dim_state)]).to(DeviceConfig.DEVICE)
        else:
            self.state_expl_idx = torch.tensor(self.args.state_expl_idx).to(DeviceConfig.DEVICE)
        ...
        if self.args.exploration_bonus_hyperstate:
            ...
            self.ret_hs = torch.zeros(size=(self.args.num_processes, 1)).to(DeviceConfig.DEVICE)
        if self.args.exploration_bonus_state:
            # for normalising the rew
            self.ret_s_rms = RunningMeanStd(shape=())
            # discounted return for each environment
            self.ret_s = torch.zeros(size=(self.args.num_processes, 1)).to(DeviceConfig.DEVICE)
        if self.args.exploration_bonus_belief:
            # for normalising the rew
            self.ret_b_rms = RunningMeanStd(shape=())
            # discounted return for each environment
            self.ret_b = torch.zeros(size=(self.args.num_processes, 1)).to(DeviceConfig.DEVICE)
        if self.args.exploration_bonus_vae_error:
            # for normalising the rew
            self.ret_v_rms = RunningMeanStd(shape=())
            # discounted return for each environment
            self.ret_v = torch.zeros(size=(self.args.num_processes, 1)).to(DeviceConfig.DEVICE)
    ...
    def reward(self,
        ...
        with torch.no_grad():
            ...
            if 0 < len(self.state_expl_idx) < state.shape[-1]:
                state = state.to(DeviceConfig.DEVICE)
            ...
            if self.args.exploration_bonus_vae_error:
                ...
                if self.args.decode_reward:
                    ...
                    if self.args.vae_use_ensemble:
                        rew_reconstruction_loss = rew_reconstruction_loss[0]
                    rew_reconstruction_loss = rew_reconstruction_loss.unsqueeze(-1)
                else:
                ...
                if self.args.decode_state:
                    ...
                    state_reconstruction_loss = vae.compute_state_reconstruction_loss(
                        ...
                        batch_actions)
                    if self.args.vae_use_ensemble:
                        state_reconstruction_loss = state_reconstruction_loss[0]
                    state_reconstruction_loss = state_reconstruction_loss.unsqueeze(-1)
                else:
                ...
                if self.args.decode_task:
                    ...
                    task_reconstruction_loss = vae.compute_task_reconstruction_loss(
                        ...
                        batch_tasks)
                    if self.args.vae_use_ensemble:
                        task_reconstruction_loss = task_reconstruction_loss[0]
                    task_reconstruction_loss = task_reconstruction_loss.unsqueeze(-1)
                else:
    ...
    def update(self, frames, iter_idx, log=True):
        ...
        if self.encoder is not None:
            ...
            state = state.to(DeviceConfig.DEVICE)
            belief = belief.to(DeviceConfig.DEVICE)
