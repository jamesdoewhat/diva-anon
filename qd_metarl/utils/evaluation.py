...
def evaluate(args,
    ...
    if encoder is not None:
        ...
        # episode_latent_means[0].append(latent_mean.detach().cpu().numpy())
        # episode_latent_logvars[0].append(latent_logvar.detach().cpu().numpy())
    else:
        latent_sample = latent_mean = latent_logvar = hidden_state = None

    # Copying the same procedure we using for logging training stats
    eval_sts_env_step_infos = defaultdict(list)
    ...
    for episode_idx in range(num_episodes):
        ...
        for step_idx in range(num_steps):
            ...
            [state, belief, task], rewards, done, infos = utl.env_step(
                envs, action, args)
                ...
                if episode_idx == num_episodes - 1:
                    for k, v in info.items():
                        ...
                        if 'env/' in k:
                            ...
                            eval_sts_env_step_infos['eval-env-fin-ep' + k[3:]].append(v)
    ...
    video_buffer = None
    if create_video:
        video_buffer = utl.video_from_images(fps=args.video_fps, images=all_images)
...
def visualise_behaviour(args,
    ...
    env, plr_components = make_vec_envs(env_name=args.env_name,
                        seed=args.seed * 42 + iter_idx,
                        num_processes=1,
                        gamma=args.policy_gamma,
                        device=DeviceConfig.DEVICE,
    ...
    for episode_idx in range(num_episodes):
        ...
        for step_idx in range(1, env._max_trial_steps + 1):
            ...
            if isinstance(state, dict):
                ...
                state = state.reshape((1, *v.shape)).to(DeviceConfig.DEVICE)
            task = task.view(-1) if task is not None else None
