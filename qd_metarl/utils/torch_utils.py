...
import random
import os
from qd_metarl.utils.config import DeviceConfig


def select_device(gpu_id):
    # if th.cuda.is_available() and gpu_id >= 0:
    if gpu_id >= 0:
        DeviceConfig.DEVICE = th.device('cuda:%d' % (gpu_id))
    else:
        DeviceConfig.DEVICE = th.device('cpu')


def tensor_item(x, dtype=np.float32, device=DeviceConfig.DEVICE):
...
def tensor(x, dtype=np.float32, device=DeviceConfig.DEVICE):

    if isinstance(x, dict):
        y = dict((k, tensor_item(v, dtype=dtype, device=device)) for k, v in x.items())
        return y
    elif isinstance(x, list):
        y = [tensor_item(item, device=device) for item in x]
        return y
    else:
        y = tensor_item(x, dtype=dtype, device=device)
        return y


def range_tensor(end, device=DeviceConfig.DEVICE):
    return th.arange(end).long().to(device)
...
class Grad:
    def __init__(self, network=None, grads=None, device=DeviceConfig.DEVICE):
