...
def simulate(args,
    ...
    num_steps = envs._max_trial_steps
    print('QD > Simulating QD solutions...')
    # reset environments
    ...
    if level_seeds is None:
        # TODO: Do we want to unsqueeze here too?
        level_seeds = ls
    else:
    ...
    for episode_idx in range(num_episodes):
        # NOTE: we treat each episode like a set of args.policy_num_steps in
        #       MetaLearner.train()
        # TODO: Never discovered why exactly this was necessary, but it is;
        #       something to do with not calling encode_running_trajectory
        ...
        for step_idx in range(num_steps):
            ...
            with torch.no_grad():
                value, action = utl.select_action(
                    args=args, policy=policy, state=prev_state, belief=belief,
                    task=task, latent_sample=latent_sample,
                    latent_mean=latent_mean, latent_logvar=latent_logvar,
                    # DEBUG: deterministic=True was original
                    deterministic=False)
            ...
            if encoder is not None:
                ...
                try:
                    latent_sample, latent_mean, latent_logvar, hidden_state = \
                        utl.update_encoding(encoder=encoder,
                            next_obs=next_state,
                            action=action,
                            reward=rew_raw,
                    ...
                    print("ERROR: update_encoding failed")
                    print('num_processes:', num_processes)
                    print('next_state.shape:', utl.shape(next_state))
                    print('action.shape:', utl.shape(action))
                    print('rew_raw.shape:', utl.shape(rew_raw))
                    print('hidden_state.shape:', utl.shape(hidden_state))
                    raise e
            ...
            # NOTE: Do not need to add to a VAE buffer like in training
        ...
        # number of rollout steps in MetaLearner.train(), where the updates
        # occur:
