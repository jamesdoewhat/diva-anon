# From https://github.com/facebookresearch/dcd/blob/main/level_replay/level_sampler.py
# 
...
class LevelSampler():
    ...
    def sample_replay_decision(self):
        print('LevelSampler.sample_replay_decision > self._proportion_filled = {}'.format(self._proportion_filled))
        if self.sample_full_distribution: 
    ...
    def sample(self, strategy=None):
        ...
        if not self.sample_full_distribution:
            ...
            if strategy == 'random':
                print('WARNING: Sampling RANDOM level for PLR.')
                seed_idx = np.random.choice(range((len(self.seeds))))
                seed = self.seeds[seed_idx]
                return int(seed)
            else:
                print('Sampling level for PLR based off objectives.')
        ...
        print('Replay decision: {}'.format(replay_decision))
        if replay_decision:
            print('Sampling REPLAY level for PLR.')
            return self._sample_replay_level()
        else:
            print('Sampling UNSEEN level for PLR.')
    ...
    def sample_weights(self):
        ...
        if self.strategy == 'random':
            ...
            weights = np.zeros_like(weights, dtype=np.float)
    ...
    def _add_seed(self, seed, score):
        ...
        if seed in self.seed2index:
            ...
            if len(empty_slots) == 0:
                print('LS - No empty slots in seed buffer')
                return False
            ...
            # TODO: Why isn't staleness updated here? Any other values missed?
    ...
    def purge_seeds(self, seeds):
        ...
        """
        # print('LS - Purging seeds: ', seeds)
        try:
            indices = [i for (i, _) in seeds]
            seeds = [seed for (_, seed) in seeds]
        except:
            print('Seeds: ', seeds)
            raise ValueError('Seeds must be in form [(index, seed), ...]')
        ...
        # print('(B) working seed set:', len(self.working_seed_set))
        for seed in seed_list:
            self.working_seed_set.discard(seed)
        # print('(A) working seed set:', len(self.working_seed_set))
    ...
    def merge_sampler(self, other_sampler):
        ...
        print(f'Added {len(other_seeds) - len(failed_seeds)}/{len(other_seeds)}'
               ' from other sampler.')
