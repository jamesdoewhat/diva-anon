# From https://github.com/facebookresearch/dcd/blob/main/level_replay/level_store.py
# 
...
class LevelStore(object):
	...
	def insert(self, level, parent_seeds=None, no_iter=False):
		if hasattr(level, '__iter__') and not no_iter:
			idx = []
	...
	def purge_seeds(self, seeds):
		""" Seeds is list of tuples: [(index, seed), ...] """
		for _, seed in seeds:
			self._remove(seed)
