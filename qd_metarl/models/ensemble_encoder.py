...
class EnsembleRNNEncoder(nn.Module):
    """
    NOTE: This class was copied from qd_metarl/models/encoder.py and modified
    to support the ensemble VAE.
    """
    def __init__(self,
                 args,
                 # network size
        ...
        """
        Initialise the *ensemble* RNN encoder.

        NOTE: All we have done differently in __init__ is: 
        - add ensemble_size as an argument
        - add self.ensemble_size = ensemble_size
        - change the output layer to output mu and logvar for all ensemble members
        """
        super(EnsembleRNNEncoder, self).__init__()
        ...
        # NEW: Separate output layers for each ensemble member
        self.fc_mu = nn.ModuleList([nn.Linear(curr_input_dim, latent_dim) for _ in range(ensemble_size)])
        self.fc_logvar = nn.ModuleList([nn.Linear(curr_input_dim, latent_dim) for _ in range(ensemble_size)])

    def _sample_gaussian(self, mu, logvar, num=None):
        """
        Sample from a Gaussian with diagonal covariance matrix using the
        reparameterisation trick.

        NOTE: has been verified to sample from each ensemble member independently
        (the original code incidentally already supported this).
        """
        # Input shape of mu and logvar: (BS x latent_dim * ensemble_size)
        # Modify to sample from each ensemble member independently
        if num is None:
            # NOTE: no change was needed from original code!
            std = torch.exp(0.5 * logvar)
            eps = torch.randn_like(std)    
            sampled = eps.mul(std).add_(mu)

            # Here, sampled will have shape (BS x latent_dim x ensemble_size)
            # With each ensemble member sampled independently
            return sampled
        else:
    ...
    def reset_hidden(self, hidden_state, done):
        ...
        NOTE: No modification was needed to support the ensemble.
        """
    ...
    def prior(self, batch_size, sample=True):
        ...
        NOTE: This function was modified to support the ensemble.
        """
        ...
        for i in range(self.ensemble_size):
            mu_i = self.fc_mu[i](h)
            logvar_i = self.fc_logvar[i](h)
            mu_list.append(mu_i)
            logvar_list.append(logvar_i)
        # Stack the outputs to get tensors of shape [BS x latent_dim x ensemble_size]
        latent_mean = torch.stack(mu_list, dim=-1)
        latent_logvar = torch.stack(logvar_list, dim=-1)
        # Reshape to (BS x latent_dim * ensemble_size)
        latent_mean = latent_mean.reshape((*latent_mean.shape[0:-2], self.latent_dim * self.ensemble_size))
        latent_logvar = latent_logvar.reshape((*latent_logvar.shape[0:-2], self.latent_dim * self.ensemble_size))
        if sample:
            latent_sample = self.reparameterise(latent_mean, latent_logvar)
        else:
            latent_sample = latent_mean
        # Latent sample still shape (BS x latent_dim * ensemble_size)
    ...
    def forward(self, actions, states, rewards, hidden_state, return_prior, 
        ...
        logvar_list = []
        for i in range(self.ensemble_size):
            mu_i = self.fc_mu[i](gru_h)
            logvar_i = self.fc_logvar[i](gru_h)
            mu_list.append(mu_i)
            logvar_list.append(logvar_i)
        # Stack the outputs to get tensors of shape [BS x latent_dim x ensemble_size]
        latent_mean = torch.stack(mu_list, dim=-1)
