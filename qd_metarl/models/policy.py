...
from qd_metarl.models.popart import PopArt
try:
...
class Policy(nn.Module):
    def __init__(self,
                 ...
                 state_is_dict=False,  # TODO: redundant?,
                 use_popart=False,
                 use_beta=False,
                 ):
        """
        ...
        if self.pass_state_to_policy and self.use_state_encoder:
            self.state_encoder = state_feature_extractor(dim_state, self.args.policy_state_embedding_dim, self.activation_function)
            curr_input_dim += self.args.policy_state_embedding_dim
        else:
    ...
    def forward(self, state, latent, belief, task):
        ...
        if self.pass_state_to_policy:
            ...
            state = torch.zeros(0, ).to(DeviceConfig.DEVICE)
        ...
        if self.pass_latent_to_policy:
            ...
            latent = torch.zeros(0, ).to(DeviceConfig.DEVICE)
        ...
        if self.pass_belief_to_policy:
            ...
            belief = torch.zeros(0, ).to(DeviceConfig.DEVICE)
        ...
        if self.pass_task_to_policy:
            ...
            task = torch.zeros(0, ).to(DeviceConfig.DEVICE)
...
class DiagGaussian(nn.Module):
    def __init__(self, num_inputs, num_outputs, init_std, norm_actions_pre_sampling):
        ...
        self.min_std = torch.tensor([1e-6]).to(DeviceConfig.DEVICE)
