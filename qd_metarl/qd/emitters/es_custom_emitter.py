...
from qd_metarl.qd.emitters.cma_es import CMAEvolutionStrategy as _CMAEvolutionStrategy
...
class EvolutionStrategyCustomEmitter(EvolutionStrategyEmitter):
    ...
    def __init__(self,
        ...
        # TODO: Try different values of sigma0
        super().__init__(
            archive, 
            x0=x0, 
            sigma0=sigma0,
    ...
    def ask(self):
        ...
        if self.sols_emitted < self.initial_population:
            ...
            # print('Still generating random solutions to fill initial_population...')
        ...
        raw_sols = self._opt.ask(np.full_like(self.lower_bounds, -np.inf),
                                 np.full_like(self.upper_bounds, np.inf))
        #######################################################################
    ...
    def tell(self, 
             ...
             value_batch,
             metadata_batch=None):
        if self._last_ask_random:  # No update on random solutions
            return
        # Call super
        super().tell(solution_batch, objective_batch, measures_batch, status_batch, value_batch, metadata_batch)
