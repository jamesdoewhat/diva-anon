...
NOTE: This version was copied from Pyribs to add a modification so that
division by zero does not occur in the calculation of the left variable in the
tell method. This is a temporary fix until the original issue is resolved in
Pyribs.
...
class CMAEvolutionStrategy(EvolutionStrategyBase):
    ...
    def tell(self, ranking_indices, num_parents):
        ...
        # Clip self.ps to be between -1e6 and 1e6 to avoid overflow in the
        # exponentiation.
        self.ps = np.clip(self.ps, -1e6, 1e6)

        # DEBUG: Modification for numerical stability
        try:
            exponent = 2 * self.current_eval / self.batch_size
            exp_term = (1 - cs)**exponent
            left = (np.sum(np.square(self.ps)) / self.solution_dim /
                    (1 - exp_term + 1e-8))
        except FloatingPointError:
            # Underflow handling, set left to sum of squares divided by dimension
            print("Underflow encountered, using default calculation")
            try:
                left = np.sum(np.square(self.ps)) / self.solution_dim
            except Exception as exc:
                print('type(self.ps)', type(self.ps))
                print('self.ps', self.ps)
                print('min(self.ps)', np.min(self.ps))
                print('max(self.ps)', np.max(self.ps))
                raise exc

        # except Exception as e:
        #     print(f"Exception in left: {e}")
        #     print(f"self.ps: {self.ps}")
        #     print(f"self.solution_dim: {self.solution_dim}")
        #     print(f"self.current_eval: {self.current_eval}")
